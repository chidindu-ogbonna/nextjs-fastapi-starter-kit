...
from backend.loop import APIProvider, sampling_loop
...
async def test_loop():
    ...
    with mock.patch("backend.loop.Anthropic", return_value=client), mock.patch(
        "backend.loop.ToolCollection", return_value=tool_collection
    ):
